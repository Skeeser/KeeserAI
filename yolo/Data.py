...
class yoloDataset(data.Dataset):
    ...
    def encoder(self, boxes, labels):
        ...
        return target
    """
    这一部分其实也可以写成这1、先将bbox的坐标[x1, y1, x2, y2]的形式转换成[x_center, y_center, w, h]
    2、然后将labels(list)[0, 1, 4, 2, ...]和bboxes(list)[[x_center, y_center, width, height], ...]
      转换成[self.S, self.S, self.B*5+self.C]即[7, 7, 30]，代码如下：
